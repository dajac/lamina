...
(defn observer
  ...
  ([message-callback close-callback observers-callback]
	 ...
	 (try
	   ...
	   (catch Exception e
	     (log/error e "Error in message callback" (.getMessage e)))))
       (on-close [_]
	 (try
	   (when close-callback
	     (close-callback))
	   (catch Exception e
	     (log/error e "Error in close callback" (.getMessage e)))))
       (on-observers-changed [_ observers]
	 (try
	   (when observers-callback
	     (observers-callback observers))
	   (catch Exception e
	     (log/error e "Error in observers-changed callback" (.getMessage e))))))))
